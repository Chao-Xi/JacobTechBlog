# Helm Chart (`Chart` 文件结构 / `Chart.yaml` 文件 / `Chart` 依赖关系 / 通过 `charts/` 目录手动管理依赖性 / 模板 `Templates` 和值 `Values` / 使用 `Helm` 管理 `chart`)
...
**这提供了一种与所有子 `char`t 共享一个顶级变量的方法，这对设置 `metadata` 中像标签这样的属性很有用。**

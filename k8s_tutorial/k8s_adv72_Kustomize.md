...
要使用 `Kustomize`，你需要有一个原始的 `yaml` 文件来描述你想要部署到集群中的任何资源，我们这里将这些 `base` 文件存储在`./kustomize/base` 文件夹下面。
...
和前面一样，在`kustomization.yaml`文件中`的patchesStrategicMerge`下面添加这里定制的数据：

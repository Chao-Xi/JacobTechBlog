...
除了上面的一些环境变量相关的配置外，还需要一个用于注册、运行和取消注册 `Gitlab CI Runner` 的小脚本。只有当 `Pod` 正常通过 `Kubernetes`（TERM信号）终止时，才会触发转轮取消注册。
...
然后接下来我们就可以来编写一个用于真正运行 `Runner` 的控制器对象，我们这里使用 `Statefulset`。

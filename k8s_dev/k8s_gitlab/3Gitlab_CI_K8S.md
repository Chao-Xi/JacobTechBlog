...
我们的应用程序运行`8000`端口上，端口名为`http-metric`s，如果你还记得前面我们监控的课程中应该还记得我们使用`prometheus-operator`为 `Prometheus` 创建了自动发现的配置，所以我们在`annotations`里面配置上上面的这几个注释后，`Prometheus` 就可以自动获取我们应用的监控指标数据了。
